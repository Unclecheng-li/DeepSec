...
def test_network_scan_profile_planning():
    from deepsec.spear.agent.network_scan import build_nmap_command, build_nmap_plan
...
def test_build_nmap_command_deescalates_without_root(monkeypatch):
    ...
    monkeypatch.setattr("deepsec.spear.agent.network_scan.nmap_has_raw_socket_access", lambda: False)
    command = build_nmap_command("/usr/bin/nmap", "192.168.56.10", plan)
...
def test_network_scan_adaptive_uses_prior_ports():
    from deepsec.spear.agent.network_scan import build_nmap_plan
...
def test_detect_connected_wifi_target_uses_active_wireless_ipv4(monkeypatch):
    import deepsec.spear.agent.network_scan as network_scan
...
def test_parse_nmap_xml_structured_ranks_weak_links():
    from deepsec.spear.agent.network_scan import parse_nmap_xml_structured, summarize_network_scan
