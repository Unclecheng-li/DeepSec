...
CONFIG_DIR = Path(os.environ.get("DEEPSEC_CONFIG_DIR", str(Path.home() / ".deepsec")))
...
def openai_default_headers() -> dict[str, str]:
    return {"User-Agent": os.environ.get("DEEPSEC_LLM_USER_AGENT", DEFAULT_OPENAI_USER_AGENT)}
...
def load_config() -> VulnClawConfig:
    ...
    # Auto-complete base_url / model from the provider preset when the user
    # hasn't set them explicitly (or left the OpenAI defaults while selecting a
    # different provider). This keeps `config set llm.provider deepseek` working
    # without also requiring an explicit base_url, and makes the DeepSeek preset
    # (https://api.deepseek.com/v1) actually take effect.
...
def _overlay_env(config: VulnClawConfig) -> VulnClawConfig:
    ...
    Supported env vars (prefix DEEPSEC_):
    ...
    """
    # ── LLM ──────────────────────────────────────────────────────────
    if v := os.environ.get("DEEPSEC_LLM_API_KEY"):
        config.llm.api_key = v
    if v := os.environ.get("DEEPSEC_LLM_API_KEYS"):
        keys = [k.strip() for k in v.split(",") if k.strip()]
        if keys:
            config.llm.api_keys = keys
    if v := os.environ.get("DEEPSEC_LLM_BASE_URL"):
        config.llm.base_url = v
    if v := os.environ.get("DEEPSEC_LLM_MODEL"):
        config.llm.model = v
    if v := os.environ.get("DEEPSEC_LLM_PROVIDER"):
        config.llm.provider = v
    if v := os.environ.get("DEEPSEC_LLM_MAX_TOKENS"):
        with suppress(ValueError):
            config.llm.max_tokens = int(v)
    if v := os.environ.get("DEEPSEC_LLM_MAX_CONTEXT_TOKENS"):
        with suppress(ValueError):
            config.llm.max_context_tokens = int(v)
    if v := os.environ.get("DEEPSEC_LLM_TEMPERATURE"):
        with suppress(ValueError):
            config.llm.temperature = float(v)

    # ── LLM auth mode (static / oauth) ──────────────────────────────────
    if v := os.environ.get("DEEPSEC_LLM_AUTH_MODE"):
        config.llm.auth_mode = v
    if v := os.environ.get("DEEPSEC_LLM_CHATGPT_AUTO_PROXY"):
        config.llm.chatgpt_auto_proxy = v.lower() in ("1", "true", "yes", "on")

    # ── Session ──────────────────────────────────────────────────────
    if v := os.environ.get("DEEPSEC_SESSION_OUTPUT_DIR"):
        config.session.output_dir = Path(v)
    if v := os.environ.get("DEEPSEC_RUNS_DIR"):
        config.session.runs_dir = Path(v)
    if v := os.environ.get("DEEPSEC_SESSION_RUNS_DIR"):
        config.session.runs_dir = Path(v)
    if v := os.environ.get("DEEPSEC_SESSION_AUTO_SAVE"):
        config.session.auto_save = v.lower() in ("1", "true", "yes", "on")
    if v := os.environ.get("DEEPSEC_SESSION_REPORT_FORMAT"):
        config.session.report_format = v
    if v := os.environ.get("DEEPSEC_SESSION_MAX_ROUNDS"):
        with suppress(ValueError):
            config.session.max_rounds = int(v)
    if v := os.environ.get("DEEPSEC_SESSION_SHOW_THINKING"):
        config.session.show_thinking = v.lower() in ("1", "true", "yes", "on")
    if v := os.environ.get("DEEPSEC_SESSION_REPL_PARALLEL_ENABLED"):
        config.session.repl_parallel_enabled = v.lower() in ("1", "true", "yes", "on")
    if v := os.environ.get("DEEPSEC_SESSION_REPL_PARALLEL_AGENTS"):
        with suppress(ValueError):
            config.session.repl_parallel_agents = int(v)
    if v := os.environ.get("DEEPSEC_SESSION_REPL_PARALLEL_DEPTH"):
        with suppress(ValueError):
            config.session.repl_parallel_depth = int(v)
    if v := os.environ.get("DEEPSEC_SESSION_REPL_PARALLEL_WORKER_ROUNDS"):
        with suppress(ValueError):
            config.session.repl_parallel_worker_rounds = int(v)
    if v := os.environ.get("DEEPSEC_SESSION_REPL_PARALLEL_SURFACE_LIMIT"):
        with suppress(ValueError):
            config.session.repl_parallel_surface_limit = int(v)
    if v := os.environ.get("DEEPSEC_SESSION_STALE_ROUNDS_THRESHOLD"):
        with suppress(ValueError):
            config.session.stale_rounds_threshold = int(v)

    # ── Session: 推理状态 / 反思引擎 / 插件运行时 ──────────────
    _truthy = ("1", "true", "yes", "on")
    if v := os.environ.get("DEEPSEC_SESSION_REASONING_STATE_ENABLED"):
        config.session.reasoning_state_enabled = v.lower() in _truthy
    if v := os.environ.get("DEEPSEC_SESSION_REFLEXION_ENABLED"):
        config.session.reflexion_enabled = v.lower() in _truthy
    if v := os.environ.get("DEEPSEC_SESSION_REFLEXION_MAX_SAME_VULN_FAILS"):
        with suppress(ValueError):
            config.session.reflexion_max_same_vuln_fails = int(v)
    if v := os.environ.get("DEEPSEC_SESSION_REFLEXION_MAX_TOTAL_NO_PROGRESS"):
        with suppress(ValueError):
            config.session.reflexion_max_total_no_progress = int(v)
    if v := os.environ.get("DEEPSEC_SESSION_ESCALATION_MAX_LEVEL"):
        with suppress(ValueError):
            config.session.escalation_max_level = int(v)
    if v := os.environ.get("DEEPSEC_SESSION_PLUGIN_RUNTIME_ENABLED"):
        config.session.plugin_runtime_enabled = v.lower() in _truthy
    if v := os.environ.get("DEEPSEC_SESSION_PLUGIN_DEFAULT_TIMEOUT"):
        with suppress(ValueError):
            config.session.plugin_default_timeout = int(v)
    if v := os.environ.get("DEEPSEC_SESSION_PLUGIN_MAX_REQUESTS_PER_TARGET"):
        with suppress(ValueError):
            config.session.plugin_max_requests_per_target = int(v)
    if v := os.environ.get("DEEPSEC_SESSION_EVIDENCE_MIN_REPORT_LEVEL"):
        config.session.evidence_min_report_level = v

    # ── Safety ───────────────────────────────────────────────────────
    if v := os.environ.get("DEEPSEC_SAFETY_PYTHON_EXECUTE_ENABLED"):
        config.safety.enable_python_execute = v.lower() in ("1", "true", "yes", "on")
    if v := os.environ.get("DEEPSEC_SAFETY_PYTHON_EXECUTE_RESTRICTED"):
        config.safety.python_execute_restricted = v.lower() in ("1", "true", "yes", "on")
    if v := os.environ.get("DEEPSEC_SAFETY_PYTHON_EXECUTE_MODE"):
        config.safety.python_execute_mode = v
    if v := os.environ.get("DEEPSEC_SAFETY_PYTHON_EXECUTE_MAX_LINES"):
        with suppress(ValueError):
            config.safety.python_execute_max_lines = int(v)
    if v := os.environ.get("DEEPSEC_SAFETY_PYTHON_EXECUTE_SHOW_WARNING"):
        config.safety.python_execute_show_warning = v.lower() in ("1", "true", "yes", "on")
    if v := os.environ.get("DEEPSEC_SAFETY_PYTHON_EXECUTE_MAX_OUTPUT_CHARS"):
        with suppress(ValueError):
            config.safety.python_execute_max_output_chars = int(v)
    if v := os.environ.get("DEEPSEC_SAFETY_PYTHON_EXECUTE_AUDIT_ENABLED"):
    ...
    # (DEEPSEC_RECON_FOFA_KEY); short form wins if both are set.
    for field, names in {
        "fofa_email": ("FOFA_EMAIL", "DEEPSEC_RECON_FOFA_EMAIL"),
        "fofa_key": ("FOFA_KEY", "DEEPSEC_RECON_FOFA_KEY"),
        "hunter_key": ("HUNTER_KEY", "DEEPSEC_RECON_HUNTER_KEY"),
        "quake_key": ("QUAKE_KEY", "DEEPSEC_RECON_QUAKE_KEY"),
        "zoomeye_key": ("ZOOMEYE_KEY", "DEEPSEC_RECON_ZOOMEYE_KEY"),
        "shodan_key": ("SHODAN_KEY", "DEEPSEC_RECON_SHODAN_KEY"),
        "zerozone_key": ("ZEROZONE_KEY", "DEEPSEC_RECON_ZEROZONE_KEY"),
    }.items():
...
def _apply_provider_defaults(config: VulnClawConfig) -> None:
    """Fill base_url/model from the provider preset when not explicitly set.

    Without this, selecting a non-OpenAI provider via ``config set llm.provider``
    leaves base_url at the OpenAI default, so requests would hit the wrong
    endpoint. We only fill when the value is empty or still equals the OpenAI
    default while a different provider is selected; an explicit user value
    (including a custom endpoint) is always preserved.
    """
    provider = (config.llm.provider or "").strip().lower()
    if not provider:
        return
    try:
        preset = PROVIDER_PRESETS.get(LLMProvider(provider))
    except ValueError:
        preset = PROVIDER_PRESETS.get(LLMProvider("custom"))
    if not preset:
        return

    openai_default_base = "https://api.openai.com/v1"
    openai_default_model = "gpt-4o"
    if not config.llm.base_url or (
        config.llm.base_url == openai_default_base and provider != "openai"
    ):
        preset_base = preset.get("base_url")
        if preset_base:
            config.llm.base_url = preset_base
    if not config.llm.model or (
        config.llm.model == openai_default_model and provider != "openai"
    ):
        preset_model = preset.get("default_model")
        if preset_model:
            config.llm.model = preset_model
