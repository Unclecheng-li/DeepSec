...
from deepsec import __version__, headless
from deepsec.spear.agent.constraint_policy import validate_action_constraints
from deepsec.spear.agent.input_analysis import extract_task_constraints

# === Stream Output Renderer ===
# 修改者: Nyaecho
# 修改时间: 2026-07-08
# 修改原因: S2 修复 — 共享辅助函数已移至 cli/_helpers.py。
from deepsec.spear.legacy_cli._helpers import (
    TerminalStreamSink,
...
from deepsec.spear.legacy_cli.manual import available_topics, render_manual
from deepsec.config.settings import (
    RUNS_DIR,
...
from deepsec.config.token_provider import has_llm_credentials
from deepsec.i18n import _
from deepsec.spear.repl_runner import run_repl_call
from deepsec.target_state.store import (
...
def _emit_solve_report_if_completed(agent: Any, config: Any) -> str:
    ...
    if session is not None and not getattr(session, "solve_auto_report", True):
        return ""
    try:
        from deepsec.spear.report.solve_report import generate_solve_report
...
def _make_repl_prompt_session() -> Any:
    ...
    """
    debug = bool(os.environ.get("DEEPSEC_DEBUG_INPUT"))
    try:
        ...
        from deepsec.spear.legacy_cli.tui import build_repl_slash_completer, build_repl_slash_style
...
def _run_repl_command(name: str, args: str, agent: Any, config: Any) -> Any:
    ...
    """
    if name == "config":
        from deepsec.spear.legacy_cli.tui import run_config_tui
...
def _repl_switch_language(args: str, agent: Any, config: Any) -> Any:
    ...
    from deepsec.spear.legacy_cli.tui import _SUPPORTED_LANGUAGES, rebuild_translations
    from deepsec.i18n import init_i18n
...
def _run_repl() -> None:
    ...
    from deepsec.mcp.lifecycle import MCPLifecycleManager
    ...
    while True:
        try:
            ...
            if user_input.startswith("/"):
                from deepsec.spear.legacy_cli.tui import dispatch_repl_slash
            ...
            if cmd_lower in ("exit", "quit", "q"):
                ...
                from deepsec.spear.agent.core import PersistentCycleResult
...
def run(
    ...
    runs_dir: Optional[str] = typer.Option(
        None, "--runs-dir", help="Run-directory root (overrides DEEPSEC_RUNS_DIR)"
    ),
    ...
    async def _run():
        async def runner(agent, shared_config):
            ...
            if selected_engine == "solve":
                ...
                from deepsec.spear.agent.team import run_team_pentest
...
def network_scan(
    ...
    if not scan_target:
        from deepsec.spear.agent.network_scan import detect_connected_wifi_target
    ...
    async def _run():
        async def runner(agent, _config):
            ...
            if parallel_agents > 1:
                from deepsec.spear.agent.parallel_agents import run_parallel_pentest
...
def report(
    ...
    if target_mode:
        from deepsec.spear.report.generator import generate_report_from_target_state
        ...
        from deepsec.spear.report.generator import generate_report_from_file
    ...
    if pdf:
        ...
        from deepsec.spear.report.pdf_exporter import export_pdf
...
def config_provider(
    ...
    # Switch provider
    from deepsec.config.schema import PROVIDER_PRESETS, LLMProvider
...
def init() -> None:
    ...
    from deepsec.config.settings import ensure_dirs
...
def login(
    ...
    """
    from deepsec.config.token_provider import (
        CHATGPT_CLIENT_ID,
        CHATGPT_TOKEN_URL,
        OAuthError,
    ...
    if proxy_url:
        ...
        console.print(
            ...
            "so you can adjust llm.model or the DEEPSEC_CHATGPT_* env vars.[/]"
        )
...
def logout() -> None:
    ...
    from deepsec.config.token_provider import logout_oauth
...
def doctor() -> None:
    ...
    from deepsec.mcp.diagnostics import get_mcp_diagnostics
...
def plugins_list(
    ...
    from deepsec.plugins import registry
...
def plugins_run(
    ...
    from deepsec.plugins import PluginContext, PluginStage, create_builtin_runtime
    from deepsec.plugins.integration import merge_plugin_results_into_session
    ...
    if session_file:
        ...
        from deepsec.spear.agent.context import SessionState
...
def kb_update() -> None:
    ...
    from deepsec.kb.updater import seed_knowledge_base
...
def kb_status() -> None:
    ...
    from deepsec.kb.retriever import KnowledgeRetriever, RetrieverStatus
    from deepsec.kb.store import KnowledgeStore
...
def _resume_saved_run(
    ...
    *,
    ...
    from deepsec.spear.run_context import RunCorruptError, load_run_context
...
def _auto_save_recon_report(agent, user_input: str, config) -> None:
    ...
    try:
        ...
        from deepsec.spear.report.generator import generate_report
...
def tui(
    ...
    from deepsec.spear.legacy_cli.tui import (
        MODES,
    ...
    if once and target:
        from deepsec.spear.legacy_cli.tui import render_tui_home
...
def web(
    ...
    from deepsec.web.app import FASTAPI_AVAILABLE
    ...
    from deepsec.web.app import create_app
    from deepsec.web.auth import generate_token
