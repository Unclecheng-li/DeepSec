...
if TYPE_CHECKING:
    from deepsec.spear.agent.agent_context import AgentContext

from urllib.parse import quote, urljoin, urlparse

import httpx

from deepsec.spear.agent.agent_state import clip_text, one_line
from deepsec.spear.agent.constraint_policy import validate_tool_action
from deepsec.spear.agent.network_scan import (
...
from deepsec.spear.agent.roles import role_tool_violation, tool_allowed_for_role
from deepsec.spear.agent.tool_result_overrides import set_raw_tool_output_override
from deepsec.config.source_render import (
...
#          此处重新导出以保持向后兼容。
from deepsec.config.url_utils import infer_port_from_url  # noqa: F401 — re-export
from deepsec.spear.intel.tools import (
    INTEL_TOOL_NAMES,
    dispatch_intel_tool,
    intel_tool_schemas,
)
...
def resolve_traffic_store(agent: AgentContext) -> Any:
    ...
    """
    from deepsec.traffic.paths import resolve_traffic_store as _resolve
...
async def execute_mcp_tool(agent: AgentContext, tool_name: str, args: dict[str, Any]) -> str:
    ...
    if tool_name == "load_skill_reference":
        try:
            from deepsec.spear.skills.loader import load_skill_reference
    ...
    if tool_name == "crypto_decode":
        try:
            from deepsec.spear.skills.crypto_tools import execute as crypto_execute
    ...
    if tool_name in {"space_search", "subdomain_enum", "js_recon", "dir_enum", "unauth_test"}:
        from deepsec.spear.agent import recon_tools
...
def _write_python_audit(
    ...
    *,
    ...
    try:
        ...
        from deepsec.config.settings import PYTHON_EXECUTE_AUDIT_FILE, ensure_dirs
...
async def execute_python(agent: AgentContext, args: dict[str, Any]) -> str:
    ...
    try:
        ...
        env = {**{k: v for k, v in os.environ.items() if not k.startswith("DEEPSEC_")}, **base_env} if mode == "trusted-local" else base_env
