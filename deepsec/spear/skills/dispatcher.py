...
from deepsec.spear.skills.loader import list_core_skills, list_specialized_skills, load_skill_by_name
...
class SkillDispatcher:
    ...
    :class:`~deepsec.spear.skills.resolver.SkillResolver`: ``dispatch`` returns the
    resolver's primary skill for older callers, while :meth:`resolve` exposes
    the full :class:`~deepsec.spear.skills.resolver.SkillSelection` bundle.
    """

    def resolve(self, user_input: str, **kwargs: Any) -> Any:
        """Resolve user input into a full :class:`SkillSelection` bundle."""
        from deepsec.spear.skills.resolver import SkillQuery, SkillResolver
